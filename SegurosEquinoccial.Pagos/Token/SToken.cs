...
namespace SegurosEquinoccial.Pagos.Token
{
    public class SToken : ServiceAuthorizationManager
    {
        private static string cadenaSeguridad = "SegurosEquinoccial//Pagos//{2019}";
            ...
            if (!result)
            {
                ...
                if (message.Properties.TryGetValue(HttpRequestMessageProperty.Name, out httpRequestMessageObject))
                {
                    ...
                    if (!string.IsNullOrEmpty(httpRequestMessage.Headers["Authorization"]))
                    {
                        ...
                        result = new SToken().ValidacionAutorizacion(authorization);
                    }
                }
            }
